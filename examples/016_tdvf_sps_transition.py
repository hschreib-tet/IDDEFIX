...
def create_initial_poles(
    ...
    for frequency in frequencies:
        pole = -complex_damping + 1j * 2.0 * np.pi * frequency
...
def print_poles(poles):
    ...
    for index, pole in enumerate(sorted_poles):
        if pole.real < 0.0:
            decay_time_ns = -1.0 / pole.real * 1.0e9
        else:
            decay_time_ns = np.inf

        frequency = pole.imag / (2.0 * np.pi)

        print(f"{index:5d}{pole.real:22.8e}{frequency:24.8e}{decay_time_ns:22.6e}")
...
def main():
    ...
    maximum_time_step_deviation = np.max(np.abs(time_differences - time_step))

    relative_time_step_deviation = maximum_time_step_deviation / abs(time_step)

    print(f"Maximum relative time-step deviation: {relative_time_step_deviation:.6e}")

    if relative_time_step_deviation > 1.0e-6:
        raise ValueError("The CST time axis is not sufficiently equidistant.")

    # Reconstruct an exactly equidistant numerical time axis.
    times = np.arange(original_times.size, dtype=float) * time_step
    ...
    input_signal = np.exp(-0.5 * (cst_times / bunch_sigma) ** 2)
    ...
    fit_end_time = fit_end_cst_time - original_times[0]
    ...
    print(f"Time step: {time_step * 1.0e12:.4f} ps")
    print(f"Fit interval: 0 to {fit_end_time * 1.0e9:.2f} ns")
    print(
        f"Validation interval: {fit_end_time * 1.0e9:.2f} to {times[-1] * 1.0e9:.2f} ns"
    )
    ...
    continuation_consistency_error = np.linalg.norm(
        reconstructed_normalized[fit_mask] - result.fitted_output
    ) / np.linalg.norm(result.fitted_output)

    print(f"Continuation consistency error: {continuation_consistency_error:.6e}")

    reconstructed_wake = reconstructed_normalized * wake_scale

    # CST wake in the original spatial transverse normalization.
    cst_wake_for_fft = wake / c_light

    # TD-VF reconstruction in the same normalization.
    tdvf_wake_for_fft = reconstructed_wake.real / c_light

    cst_frequencies, cst_wake_transform = iddefix.compute_fft(
        cst_times,
        cst_wake_for_fft,
    )

    tdvf_fft_frequencies, tdvf_wake_transform = iddefix.compute_fft(
        cst_times,
        tdvf_wake_for_fft,
    )

    cst_finite_impedance = -1j * cst_wake_transform

    tdvf_finite_impedance = -1j * tdvf_wake_transform

    if not np.allclose(
        cst_frequencies,
        tdvf_fft_frequencies,
    ):
        raise RuntimeError("CST and TD-VF FFT frequency axes differ.")
    ...
    axes[0].set_title("SPS-transition CST wake: TD-VF extrapolation")
    axes[0].set_xlabel("Time relative to selected start [ns]")
    axes[0].set_ylabel("Dipolar transverse wake")
    axes[0].grid(True)
    axes[0].legend()
    ...
    axes[1].set_xlabel("Time relative to selected start [ns]")
    axes[1].set_ylabel("Dipolar transverse wake")
    axes[1].grid(True)
    axes[1].legend()
    ...
    axes[2].set_title("Absolute error normalized by maximum CST wake")
    axes[2].set_xlabel("Time relative to selected start [ns]")
    axes[2].set_ylabel(
    ...
    pole_axis.set_xlabel(r"$\operatorname{Re}(p)$ [$10^9$ s$^{-1}$]")
    pole_axis.set_ylabel(r"$\operatorname{Im}(p)/(2\pi)$ [GHz]")
    pole_axis.grid(True)
    pole_axis.legend()
    ...
    tdvf_fully_decayed_impedance = (
        -1j * wake_scale * input_signal_area / c_light * tdvf_transfer_function
    )
    ...
    angular_frequencies = 2.0 * np.pi * cst_frequencies

    gaussian_bunch_spectrum = np.exp(-0.5 * (angular_frequencies * bunch_sigma) ** 2)

    tdvf_fully_decayed_wake_potential_impedance = (
        tdvf_fully_decayed_impedance * gaussian_bunch_spectrum
    )
    ...
    impedance_axes[0].plot(
        cst_frequencies[frequency_mask],
        tdvf_fully_decayed_wake_potential_impedance.real[frequency_mask],
        color="tab:blue",
        linestyle=":",
        linewidth=1.5,
    ...
    impedance_axes[1].plot(
        cst_frequencies[frequency_mask],
        tdvf_fully_decayed_wake_potential_impedance.imag[frequency_mask],
        color="tab:blue",
        linestyle=":",
        linewidth=1.5,
        label="TD-VF analytical wake-potential impedance",
    ...
    impedance_axes[2].semilogy(
        cst_frequencies[frequency_mask],
        np.maximum(
            np.abs(tdvf_fully_decayed_wake_potential_impedance[frequency_mask]),
            np.finfo(float).eps,
        ),
        color="tab:blue",
        linestyle=":",
    ...
    impedance_axes[0].set_ylabel(r"$\operatorname{Re}(Z_\perp)$")
    impedance_axes[1].set_ylabel(r"$\operatorname{Im}(Z_\perp)$")
    impedance_axes[2].set_ylabel(r"$|Z_\perp|$")
